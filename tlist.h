...
TList<T> operator+(const TList<T>& t1, const TList<T>& t2){
    TListIterator<T> iterator = t2.GetIterator();

    TList<T> temporary = t1; // Initialize temporary list as t1

    if(t2.GetSize() == 0){ // If t2 is empty, return t1 (concatenating nothing)
        return temporary;
    }
    // Insert the elements of t2 to the back of t1 (temporary)
    for(int i = 0; i < t2.GetSize(); i++){
        temporary.InsertBack(iterator.GetData());
        iterator.Next();
    }
    return temporary;
}
